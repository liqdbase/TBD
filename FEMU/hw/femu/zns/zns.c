...
{
    uint16_t status;
    int state = zns_get_zone_state(zone);
    switch (zns_get_zone_state(zone)) {
    ...
    case NVME_ZONE_STATE_READ_ONLY:
        ...
        break;
    default:
        printf("[FEMU] Error: Invalid Zone State Detected: %d (SLBA: %lu)\n", 
               state, zone->d.zslba);
        status = NVME_INTERNAL_DEV_ERROR; // 내부 에러로 처리
        break;
    }
...
static void zns_init(FemuCtrl *n, Error **errp)
{
    ...
    zns->zone_size_lbas = (zns->num_ch * zns->num_lun * 
                           zns->num_plane * zns->num_blk * 
                           zns->num_page * ZNS_PAGE_SIZE) / zns->lbasz;
}
